...
| `outputs/**` | 프로젝트 이관·시나리오 테스트 등 **이 지침 저장소 자체**의 이력 기록 | 규칙 문서, 개별 프로젝트의 CR 보고서 |
| `tools/**` | 린터·빌드 스크립트 | 규칙 문서(사용법 README만 허용) |

> **CR 보고서는 여기 없다.** `.clinerules`는 여러 프로젝트가 공유하는 서브모듈이라, 프로젝트별 CR을 여기 두면 서로 뒤섞인다. CR 보고서·릴리즈 노트는 각 프로젝트 저장소의 `{{paths.cr_root}}`(정본: [`06-git-rules/05-cr-id.md`](06-git-rules/05-cr-id.md))에 저장한다.
