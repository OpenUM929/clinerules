...
│   ├── print.css                      # A4 인쇄 스타일 (@page A4, max-width:640px)
│   ├── MANIFEST.txt                   # 통합 순서(파일 목록, 탭 파일 포함)
│   └── build_integrated.py            # 통합 md/html 생성 (표준 라이브러리만)
...
> **통합본**: 일일이 인쇄하지 않으려면 `integrated-manual.html`을 브라우저에서 열고 "인쇄 → PDF 저장"(용지 A4). 재생성은 `python build/build_integrated.py`.
